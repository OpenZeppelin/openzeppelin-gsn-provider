...
const { appendAddress, preconditionCodeToDescription } = require('../utils');
...
class DevRelayClient {
  ...
  async sendTransaction(payload) {
    ...
    const gasPrice = txParams.gasPrice || await this.web3.eth.getGasPrice();
    const gas = txParams.gas || await this.estimateGas(txParams, hub.options.address);
    ...
    const approvalData = await this.getApprovalData(txParams, {
      gasPrice, gas, nonce, relayHubAddress: hub.options.address
    });
  ...
  async getApprovalData(txParams, { gasPrice, gas, nonce, relayHubAddress }) {
    const approveFunction = txParams.approveFunction || this.approveFunction;
    try {
      if (typeof approveFunction === "function") {
        return await approveFunction({
          from: txParams.from,
          to: txParams.to,
          ...
          gas_limit: gas,
          nonce: nonce,
          relay_hub_address: relayHubAddress,
          relay_address: this.relayerAddress
        })
      } else {
        return '0x';
      }
