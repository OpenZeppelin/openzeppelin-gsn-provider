...
const {
  abi: RejectfulGreeterAbi,
  bytecode: RejectfulGreeterBytecode,
} = require('./build/contracts/RejectfulGreeter.json');
...
describe('GSNProvider', function() {
  ...
  context('on no relayers available', async function() {
    ...
    it('reports no registered relayers', async function() {
      const hub = await getRelayHub(this.web3)
        .deploy()
        .send({ from: this.deployer, gas: 6.5e6 });
    ...
    it('throws if canRelay failed', async function() {
      const RejectfulGreeter = new this.web3.eth.Contract(RejectfulGreeterAbi, null, {
        data: RejectfulGreeterBytecode,
      });
  ...
  context('detecting relays added', function() {
    const withoutGsnDev = relayers => relayers.filter(r => r.relayUrl !== 'http://gsn-dev-relayer.openzeppelin.com/');
    ...
    context('with a second relay added', async function() {
      beforeEach('adding second relay', async function() {
        ...
        await this.relayHub.methods.stake(this.secondRelay, 60 * 60 * 24 * 10 /* 10 days */).send({
          value: Web3.utils.toWei('1', 'ether'),
          from: this.deployer,
          useGSN: false,
        });
        await this.relayHub.methods.registerRelay(0, 'url').send({
      ...
      context('with the second relay removed', async function() {
        beforeEach('removing second relay', async function() {
          await this.relayHub.methods.removeRelayByOwner(this.secondRelay).send({
            from: this.deployer,
            useGSN: false,
          });
        });
