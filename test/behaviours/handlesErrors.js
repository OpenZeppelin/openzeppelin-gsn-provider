...
function handlesErrors(createProviderFn) {
  context('on gsn errors', function() {
    ...
    it('throws if contract execution reverts', async function() {
      await expect(this.greeter.methods.reverts().send({ from: this.signer })).to.be.rejectedWith(
        /Transaction has been reverted/,
      );
    });

    it('throws if contract pre reverts', async function() {
      await expect(this.greeter.methods.greet('Hello').send({ from: this.failsPre })).to.be.rejectedWith(
        /Transaction has been reverted/,
      );
    });

    it('throws if contract post reverts', async function() {
      await expect(this.greeter.methods.greet('Hello').send({ from: this.failsPost })).to.be.rejectedWith(
        /Transaction has been reverted/,
      );
    });

    it('throws if contract execution reverts without using GSN', async function() {
      await expect(this.greeter.methods.reverts().send({ from: this.signer, useGSN: false })).to.be.rejectedWith(
        /Transaction has been reverted/,
      );
    });
  });
  ...
  context('on gsn errors with gsn disabled by default', function() {
    ...
    it('throws if contract execution reverts explicitly using GSN', async function() {
      await expect(this.greeter.methods.reverts().send({ from: this.signer, useGSN: true })).to.be.rejectedWith(
        /Transaction has been reverted/,
      );
    });
  });
  ...
  context('on illegal gsn actions', function() {
    ...
    it('throws if attempting to create a contract', async function() {
      ...
      await expect(Greeter.deploy().send({ from: this.deployer, gas: 3e6 })).to.be.rejectedWith(/cannot deploy/i);
    });

    it('creates a contract if disables gsn', async function() {
      const Greeter = new this.web3gsn.eth.Contract(GreeterAbi, null, { data: GreeterBytecode });
      await expect(Greeter.deploy().send({ from: this.deployer, gas: 3e6, useGSN: false })).to.be.fulfilled;
    });

    it('throws if attempting to send value', async function() {
      await expect(this.greeter.methods.greet('Money').send({ from: this.signer, value: 1e14 })).to.be.rejectedWith(
        /cannot send funds/i,
      );
    });

    it('sends tx if value is zero', async function() {
      await expect(this.greeter.methods.greet('Money').send({ from: this.signer, value: 0 })).to.be.fulfilled;
    });
  ...
  context('on invalid hub', async function() {
    ...
    it('throws if hub is the zero address', async function() {
      ...
      await expect(this.greeter.methods.greet('Hello').send({ from: this.signer })).to.be.rejectedWith(/zero/);
    });
    ...
    it('throws if hub is not a contract', async function() {
      ...
      await expect(this.greeter.methods.greet('Hello').send({ from: this.signer })).to.be.rejectedWith(
        /hub is not deployed/i,
      );
    });
  ...
  context('on invalid recipient', async function() {
    ...
    it('throws if recipient does not respond hub addr', async function() {
      ...
      await expect(greeter.methods.greet('Hello').send({ from: this.signer })).to.be.rejectedWith(
        /could not get relay hub address/i,
      );
    });
    ...
    it('throws if recipient is not funded', async function() {
      ...
      await expect(greeter.methods.greet('Hello').send({ from: this.signer })).to.be.rejectedWith(/has no funds/i);
    });
