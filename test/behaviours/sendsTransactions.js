...
const {
  abi: SignatureBouncerGreeterAbi,
  bytecode: SignatureBouncerGreeterBytecode,
} = require('../build/contracts/SignatureBouncerGreeter.json');
...
function sendsTransactions(createProviderFn) {
  ...
  context('with custom sign key', function() {
    ...
    it('refuses to send meta tx if sender does not match custom signer', async function() {
      await expect(this.greeter.methods.greet('Hello').send({ from: this.signer })).to.be.rejectedWith(/unknown/i);
    });
  ...
  context('with gas estimations', function() {
    ...
    it('sends a vanilla tx without explicit gas limit', async function() {
      const receipt = await this.greeter.methods
        .greetFrom(this.signer, LONG_MESSAGE)
        .send({ from: this.signer, useGSN: false });
      assertGreetedEvent(receipt, LONG_MESSAGE);
  ...
  context('with approval data on a bouncer', function() {
    beforeEach(async function() {
      ...
      const gsnProvider = await createProviderFn.call(this, PROVIDER_URL, {
        ...HARDCODED_RELAYER_OPTS,
        approveFunction: makeApproveFunction(data => web3.eth.sign(data, approver)),
      });

      const SignatureBouncerGreeter = new this.web3.eth.Contract(SignatureBouncerGreeterAbi, null, {
        data: SignatureBouncerGreeterBytecode,
      });
      const greeter = await SignatureBouncerGreeter.deploy({ arguments: [approver] }).send({
        from: this.deployer,
        gas: 3e6,
      });
